/* Stable IndoMail mailbox skin. Keep existing mail rows mounted and only normalize their presentation. */
(() => {
  const list = document.querySelector('#mailList');
  if (!list) return;

  const style = document.createElement('style');
  style.id = 'indomail-mail-list-stable-style';
  style.textContent = `
    #mailList{padding:10px 0 94px!important}
    #mailList .mail{position:relative!important;display:grid!important;grid-template-columns:40px minmax(0,1fr) 34px!important;gap:10px!important;align-items:start!important;margin:0 0 9px!important;padding:12px!important;min-height:76px!important;border-radius:16px!important;background:linear-gradient(145deg,rgba(9,18,33,.98),rgba(6,13,24,.98))!important;border:1px solid rgba(118,150,212,.20)!important;box-shadow:0 10px 28px rgba(0,0,0,.18)!important;color:#f4f7ff!important;overflow:hidden!important}
    ...
    #mailList .mail .mail-avatar{width:40px!important;height:40px!important;border-radius:13px!important;display:grid!important;place-items:center!important;color:#fff!important;font-size:12px!important;font-weight:900!important}
  ...
  function normalize(){
    list.dataset.uiVersion='stable';
    list.querySelectorAll('.mail').forEach(row=>{
      row.classList.add('mail-v2');
      const avatar=row.querySelector('.mail-avatar');
      if(avatar){
        avatar.style.background=avatar.dataset.v2Bg||avatar.style.background||'linear-gradient(145deg,#704bff,#1bb5ed)';
        avatar.dataset.v2Bg=avatar.style.background;
      }
